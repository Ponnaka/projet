...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Récupération de Tweets</title>
	</head>
...
<?php
		...
		function recup_tweet($mc){
			...
			$requestMethod = "GET";
			$getfield = '?q='.$mc.'&lang=fr&count=50';
			...
			$bdd = new PDO('mysql:host=localhost;dbname=projet;charset=utf8','root', 'root');
 			foreach($tab_tweet as $key=>$value){
				echo "Traitement du tweet n°".$j."<br/>";
				...
				//print_r($tweet);echo '<br/>';
 				$where = '`auteur` = "'.$tweet[1].'" and `date` = "'.$tweet[0][0].'" and `heure` = "'.$tweet[0][1].'" and `post` = "'.$tweet[2].'"';
				$sql = "SELECT * FROM `post` WHERE ".$where;
				$rep = $bdd->query($sql);
				$data = $rep->fetch();
				if($data == null){
					echo "Ajout du tweet <br/>";
					$valeur = 'VALUES ("'.$tweet[1].'", "'.$tweet[0][0].'", "'.$tweet[0][1].'" ,"'.$tweet[2].'" ,"'.$tweet[0][2].'")';
					$sql = "INSERT INTO `post`( `auteur`, `date`, `heure`, `post`, `strtotime`) ".$valeur;
					$rep = $bdd->query($sql);
				}
				else{
					echo "Tweet déjà enregistré <br/>";
				}
				unset($value);
				unset($tweet);
				$j++; 
